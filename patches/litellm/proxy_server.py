...
global_max_parallel_request_retries_env: str | None = os.getenv(
...
async def update_cache(  # noqa: PLR0915
    ...
    async def _update_user_cache():
        ...
        try:
            ...
            global_proxy_spend = await user_api_key_cache.async_get_cache(
                key=f"{litellm_proxy_admin_name}:spend"
            )
            if global_proxy_spend is None:
                ...
                values_to_update_in_cache.append((f"{litellm_proxy_admin_name}:spend", increment))
        except Exception as e:
            verbose_proxy_logger.debug(
    ...
    async def _update_team_cache():
        ...
        try:
            # Fetch the existing cost for the given user
            existing_spend_obj: LiteLLM_TeamTable | None = await user_api_key_cache.async_get_cache(
                key=_id
            )
            if existing_spend_obj is None:
...
class ProxyConfig:
    ...
    def parse_search_tools(self, config: dict) -> list[SearchToolTypedDict] | None:
        ...
        """
        search_tools_raw = config.get("search_tools")
    ...
    async def load_config(  # noqa: PLR0915
        self, router: litellm.Router | None, config_file_path: str
    ):
        """
        ...
        if general_settings:
            ...
            ### CONNECT TO DATABASE ###
            database_url = general_settings.get("database_url")
            ...
            user_api_key_cache_ttl = general_settings.get("user_api_key_cache_ttl")
            ...
            ### CUSTOM API KEY AUTH ###
            ## pass filepath
            custom_auth = general_settings.get("custom_auth")
            if custom_auth is not None:
                user_custom_auth = get_instance_fn(
                    value=custom_auth, config_file_path=config_file_path
                )

            custom_key_generate = general_settings.get("custom_key_generate")
            ...
            custom_sso = general_settings.get("custom_sso")
            if custom_sso is not None:
                user_custom_sso = get_instance_fn(
                    value=custom_sso, config_file_path=config_file_path
                )

            custom_ui_sso_sign_in_handler = general_settings.get("custom_ui_sso_sign_in_handler")
            ...
            if general_settings.get("pass_through_endpoints") is not None:
            ...
            ### ALLOWED IP ###
            allowed_ips = general_settings.get("allowed_ips")
            ...
            ### RBAC ###
            rbac_role_permissions = general_settings.get("role_permissions")
        ...
        assistant_settings = config.get("assistant_settings")
        ...
        if config is not None:
            guardrails_v2 = config.get("guardrails")
        if guardrails_v2:
    ...
    async def _init_non_llm_configs(self, config: dict):
        ...
        ## AGENTS
        agent_config = config.get("agent_list")
        if agent_config:
        ...
        ## VECTOR STORES
        vector_store_registry_config = config.get("vector_store_registry")
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        _alerting_callbacks = general_settings.get("alerting")
        ...
        for _alert in _alerting_callbacks:
            if _alert == "slack":
                ...
                proxy_logging_obj.update_values(
                    alerting=general_settings.get("alerting"),
                    alerting_threshold=general_settings.get("alerting_threshold", 600),
                    alert_types=general_settings.get("alert_types"),
                    alert_to_webhook_url=general_settings.get("alert_to_webhook_url"),
                    alerting_args=general_settings.get("alerting_args"),
                    redis_cache=redis_usage_cache,
                )
            else:
    ...
    def _add_general_settings_from_db_config(
        ...
        if _general_settings is not None and "alerting" in _general_settings:
            if (
                ...
                and general_settings.get("alerting") is not None
    ...
    def _should_load_db_object(self, object_type: str | SupportedDBObjectType) -> bool:
        ...
        supported_db_objects = general_settings.get("supported_db_objects")
    ...
    async def _get_models_from_db(self, prisma_client: PrismaClient) -> list:
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy_server.py::add_deployment() - Error getting new models from DB - {str(e)}"
            )
            new_models = []
    ...
    async def add_deployment(
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:add_deployment - {str(e)}"
            )
    ...
    async def _init_sso_settings_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_sso_settings_in_db - {str(e)}"
            )
    ...
    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - {str(e)}"
            )
    ...
    async def _init_guardrails_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_guardrails_in_db - {str(e)}"
            )
    ...
    async def _init_vector_stores_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - {str(e)}"
            )
    ...
    async def _init_mcp_servers_in_db(self):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db - {str(e)}"
            )
    ...
    async def _init_agents_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_agents_in_db - {str(e)}"
            )
    ...
    async def _init_search_tools_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy.proxy_server.py::ProxyConfig:_init_search_tools_in_db - {str(e)}"
            )
    ...
    async def get_credentials(self, prisma_client: PrismaClient):
        try:
            ...
            verbose_proxy_logger.exception(
                f"litellm.proxy_server.py::get_credentials() - Error getting credentials from DB - {str(e)}"
            )
            return []
...
async def async_assistants_data_generator(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.async_assistants_data_generator(): Exception occured - {str(e)}"
        )
...
async def async_data_generator(response, user_api_key_dict: UserAPIKeyAuth, request_data: dict):
    ...
    try:
        ...
        async for chunk in proxy_logging_obj.async_post_call_streaming_iterator_hook(
            ...
            verbose_proxy_logger.debug(f"async_data_generator: received streaming chunk - {chunk}")
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.async_data_generator(): Exception occured - {str(e)}"
        )
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict,
            original_exception=e,
...
class ProxyStartupEvent:
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("litellm_jwtauth") is not None:
    ...
    async def _setup_prisma_client(
        ...
        """
        # macOS dev override: skip Prisma entirely to avoid engine/healthcheck failures
        proxy_logging_obj.info("mac override: skipping Prisma initialization")
        return None
...
async def completion(  # noqa: PLR0915
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.completion(): Exception occured - {str(e)}"
        )
        error_msg = f"{str(e)}"
...
async def moderations(
    ...
    try:
        ...
        data["model"] = (
            general_settings.get("moderation_model")  # server default
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.moderations(): Exception occured - {str(e)}"
        )
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def audio_speech(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.audio_speech(): Exception occured - {str(e)}"
        )
...
async def audio_transcriptions(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.audio_transcription(): Exception occured - {str(e)}"
        )
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def get_assistants(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.get_assistants(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.create_assistant(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.delete_assistant(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def create_threads(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.create_threads(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def get_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.get_thread(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def add_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.add_messages(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def get_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.get_messages(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def run_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.run_thread(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def model_info_v2(
    ...
    user_models_only: bool | None = fastapi.Query(
...
async def async_queue_request(
    ...
    try:
        ...
        data["model"] = (
            general_settings.get("completion_model")  # server default
            or user_model  # model name passed via cli args
...
async def update_config(config_info: ConfigYAML):  # noqa: PLR0915
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"litellm.proxy.proxy_server.update_config(): Exception occured - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                ...
                for pydantic_class in pydantic_class_list:
                    ...
                    nested_fields = [
                        FieldDetail(
                            ...
                            field_default_value=general_settings.get(sub_field),
                            stored_in_db=None,
                        )
                    ...
                    _response_obj = ConfigList(
                        ...
                        field_type=allowed_args[field_name]["type"],
                        field_description=field_info.description or "",
                        field_value=general_settings.get(field_name),
                        stored_in_db=_stored_in_db,
                        field_default_value=field_info.default,
...
async def get_config():  # noqa: PLR0915
    ...
    try:
            ...
            if _callback != "langfuse":
                ...
                for _var in env_vars:
                    ...
                    if env_variable is None:
                        ...
                        decrypted_value = decrypt_value_helper(value=env_variable, key=_var)
        ...
        verbose_proxy_logger.exception(
            f"litellm.proxy.proxy_server.get_config(): Exception occured - {str(e)}"
        )
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),

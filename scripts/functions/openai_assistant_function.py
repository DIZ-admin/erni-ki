...
class Pipe:
    class Valves(BaseModel):
        ...
        ASSISTANT_NAME: str = Field(
            default="OpenAI Assistant", description="Display name for the Assistant"
        )
        MAX_WAIT_TIME: int = Field(
            default=60, description="Maximum time to wait for Assistant response (seconds)"
        )
    ...
    def create_thread(self) -> str | None:
        ...
        try:
            ...
            response = requests.post(
                f"{self.valves.LITELLM_BASE_URL}/v1/threads", headers=headers, json={}, timeout=30
            )
    ...
    def add_message(self, thread_id: str, content: str, role: str = "user") -> str | None:
    ...
    def create_run(self, thread_id: str, instructions: str = None) -> str | None:
        ...
        try:
            ...
            run_data = {"assistant_id": self.valves.ASSISTANT_ID}
    ...
    def wait_for_run_completion(self, thread_id: str, run_id: str) -> str | None:
        ...
        try:
            ...
            for _attempt in range(self.valves.MAX_WAIT_TIME):
        ...
        try:
            ...
            if not self.valves.OPENAI_API_KEY:
                error_msg = (
                    ...
                    "Please set OPENAI_API_KEY in the function settings."
                )
            ...
            if not message_id:
                yield (
                    "data: "
                    + json.dumps({"choices": [{"delta": {"content": "❌ Failed to add message"}}]})
                    + "\n\n"
                )
                return
            ...
            yield (
                ...
                + json.dumps(
                    {
                        "choices": [
                            {"delta": {"content": "✅ Message added, starting Assistant...\n\n"}}
                        ]
                    }
                )
                + "\n\n"
            ...
            if not run_id:
                yield (
                    "data: "
                    + json.dumps({"choices": [{"delta": {"content": "❌ Failed to create run"}}]})
                    + "\n\n"
                )
                return
            ...
            for msg in messages_list:
                if msg.get("role") == "assistant":
                    ...
                    if content and len(content) > 0:
                        ...
                        if text:
                            # Stream the response
                            assistant_content = f"✅ **OpenAI Assistant Response:**\n\n{text}"
                            yield (
                                "data: "
                                + json.dumps(
            ...
            yield (
                "data: "
                + json.dumps({"choices": [{"delta": {"content": "❌ No response from Assistant"}}]})
                + "\n\n"
            )
            ...
            yield (
                "data: "
                + json.dumps({"choices": [{"delta": {"content": f"❌ Exception: {str(e)}"}}]})
                + "\n\n"
            )

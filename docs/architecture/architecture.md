...
- **Prometheus v3.0.0**: Сбор метрик с 35+ targets, 20 правил алертов (актуально
  для compose.yml)
- **Grafana v11.3.0**: Визуализация и дашборды (5 provisioned dashboards)
- **Loki v3.0.0**: Централизованное логирование через Fluent Bit (TSDB v13)
- **Fluent Bit v3.1.0**: Сбор логов
- **Alertmanager v0.27.0**: Уведомления о событиях
- **8 экспортеров**: node, postgres, redis, nginx, ollama, nvidia, cadvisor,
  blackbox
- **RAG Exporter**: SLA метрики для RAG (latency, sources)
- **Watchtower 1.7.1**: monitor-only (селективный мониторинг обновлений, без
  автообновлений)
...
- **Версия**: v3.0.0 (пин в compose.yml, обновлено 2025-11-23)
- **Порт**: 9091
- **Функции**:
...
- **Версия**: 11.3.0 (пин в compose.yml)
- **Порт**: 3000
- **Функции**:
- Визуализация метрик
...
- **Версия**: v0.27.0 (пин в compose.yml)
- **Порты**: 9093, 9094
- **Функции**:
- Управление алертами
...
- **Версия**: fluent/fluent-bit:3.1.0 (пин в compose.yml)
- **Порт**: 2020 (метрики)
- **Функции**:
...
- **Версия**: grafana/loki:3.0.0 (пин в compose.yml)
- **Порт**: 3100
- **Функции**:
...
- **Общая оценка:** [OK] ПРЕВОСХОДНО (100/100)

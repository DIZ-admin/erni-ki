...
-**Korrigierte Abfragen:**8 kritische Abfragen -**Hinzugefügte
Fallbacks:**100% -**Erfolgsrate:**100% (kein "No data") -**Performance:**<0.01s
Ausführungszeit
...
**Problem:**Keine 5xx Fehler im System, daher leere Antwort.**Lösung:**Fallback
auf 0 (keine Fehler).
...
1.**Immer Fallbacks verwenden:**`or vector(DEFAULT_VALUE)` verhindert "No data"
Panels. 2.**Rate statt Increase:**Für Graphen `rate()` verwenden, für Zähler
`increase()`. 3.**Zeitfenster:**`[5m]` ist Standard für ERNI-KI (glättet
Spikes). 4.**Quantile:**`histogram_quantile(0.95, ...)` für Latenzmessungen
(ignoriert Ausreißer). 5.**Labels:**Sparsam verwenden, um Kardinalität niedrig
zu halten.

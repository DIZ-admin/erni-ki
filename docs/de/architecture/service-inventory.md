...
| ------------ | ---------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `watchtower` | Auto-Updates, Cleanup alter Images | `127.0.0.1:8091->8080` (nur localhost) | `env/watchtower.env`, Docker socket, Secret (Watchtower API token via Docker secret)  | API nur mit Token; Limits `mem_limit=256M`, `mem_reservation=128M`, `cpus=0.2`, `oom_score_adj=500`. |
| `db`         | PostgreSQL 17 + pgvector           | Nur intern                             | `env/db.env`, Docker secret (DB password), `postgresql.conf`, Daten `./data/postgres` | Kein Auto-Update; Healthcheck `pg_isready`.                                                          |
| `redis`      | Cache/Queues/Rate Limit            | Nur intern                             | `env/redis.env`, `./conf/redis/redis.conf`, Daten `./data/redis`                      | Watchtower erlaubt (`cache-services`); pinned `redis:7.0.15-alpine` (RDB v12 inkompatibel).          |
...
| `litellm`     | LiteLLM Proxy (Thinking Tokens)                                                                         | `4000:4000`               | `env/litellm.env`, `./conf/litellm/config.yaml`, Daten `./data/litellm`, entrypoint `scripts/entrypoints/litellm.sh`, Docker secrets (LiteLLM/OpenAI keys), hängt von `db`, `ollama`                       | Auto-Update an; Limits `mem_limit=12G`, `mem_reservation=6G`, `cpus=1.0`, `oom_score_adj=-300`.                        |
...
| `openwebui`   | Haupt-UI (Next.js)                                                                                      | via Nginx (`8080` intern) | `env/openwebui.env`, Daten `./data/openwebui`, `./data/docling/shared`, entrypoint `scripts/entrypoints/openwebui.sh`, Docker secrets (DB password, LiteLLM API key, OpenWebUI secret key), GPU via `.env` | Watchtower an; Limits `mem_limit=8G`, `mem_reservation=4G`, `cpus=4`, `oom_score_adj=-600`.                            |
...
| `postgres-exporter`       | Postgres-Metriken     | `127.0.0.1:9188->9188`      | DSN aus Docker secret (Postgres DSN), hängt von `db`                | Lokal.                                                                                    |

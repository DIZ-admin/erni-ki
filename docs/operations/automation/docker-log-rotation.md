...
1. **Использовать глобальную настройку** через `/etc/docker/daemon.json` - проще
   и единообразно
2. **Настроить мониторинг** размера логов через `monitor-disk-space.sh`
3. **Периодически проверять** размер логов:
   `sudo du -sh /var/lib/docker/containers/`
4. **Не устанавливать слишком маленькие значения** - можно потерять важные логи
   при диагностике
5. **Пересоздать контейнеры** после изменения настроек для применения

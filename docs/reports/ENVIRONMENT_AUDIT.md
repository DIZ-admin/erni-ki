...
- macOS с PEP 668: предпочтительно использовать venv; глобальная установка требует `--break-system-packages`.
- Если без GPU: выставить заглушки в `.env` (`DOCLING_GPU_VISIBLE_DEVICES=` и т.п.) перед запуском стека.
